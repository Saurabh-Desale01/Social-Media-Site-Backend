...
import com.example.Social.Media.Site.response.CreateChatRequest;
...
public class ChatController {
	...
	public Chat createChat(@RequestHeader("Authorization") String jwt,
			@RequestBody CreateChatRequest chatReq) throws Exception {
		
		User reqUser = userService.findUserByJwt(jwt);
		
		User user2 = userService.findUserById(chatReq.getUserId());
